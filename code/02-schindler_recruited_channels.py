...
for index, row in tqdm(seizure_table.iterrows(), total=seizure_table.shape[0]):
    pt = row['Patient']
    ...
    if not os.path.exists(ospj(pt_data_path, f"Shat_matrix_sz-{sz_num}_period-ictal.pkl")):
        ...
        fname = f"raw_signal_period-preictal_sz-{sz_num}.pkl"
        pi_df = pd.read_pickle(ospj(pt_data_path, fname))
        ...
        fname = f"raw_signal_period-ictal_sz-{sz_num}.pkl"
        sz_df = pd.read_pickle(ospj(pt_data_path, fname))
    ...
    # recruited.to_csv(ospj(pt_data_path, f"recruited_schindler_sz-{sz_num}.csv"))
    np.save(ospj(pt_data_path, f"recruited_schindler_sz-{sz_num}.npy"), np.array(recruited))
